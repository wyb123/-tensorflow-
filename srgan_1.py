...
real_X = tf.placeholder(tf.float32, shape=[None, mnist_dim])
random_X = tf.placeholder(tf.float32, shape=[None, random_dim])
random_Y = G(random_X)
...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        for i in range(5):
            ...
            _, D_loss_ = sess.run([D_solver, D_loss], feed_dict={real_X: width, random_X: height})
