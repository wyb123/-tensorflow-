...
def load_training_set():
    path = "./TensorFlow_rumen/image"
    data = []
    dirlist = os.listdir(path)#载入训练集
    for each in dirlist:
        if each.endswith("_1.jpg"):
            img1 = cv2.resize(cv2.imread(path + each.split("_")[0] + "_1.jpg", cv2.IMREAD_GRAYSCALE), (width, height))
            img2 = cv2.resize(cv2.imread(path + each.split("_")[0] + "_1.jpg", cv2.IMREAD_GRAYSCALE), (width, height))
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "train":
        ...
        for epoch in range(training_epochs):
            ...
            if epoch % 100 == 0:
                ...
                cv2.imwrite("./data/output/srgan_" + str(epoch) + ".jpg", check_imgs.reshape((width, height)) * 255)
                cv2.imwrite("./data/output/srgan_" + str(epoch) + "_ori_x.jpg",
                            input_x_data[0].reshape((width, height)) * 255)
                cv2.imwrite("./data/output/srgan_" + str(epoch) + "_ori_y.jpg",
                            np.reshape(subdata[0][1], (width, height)) * 255)

            # if epoch % 500 == 0 and epoch > 0:
            #     saver.save(sess, "./checkPoint", epoch)
    else:
        input_x_data = cv2.resize(cv2.imread("./image/input.jpg", cv2.IMREAD_GRAYSCALE), (width, height)).reshape((1, g_ho_size)) / 255
        check_imgs = sess.run(fake_y, feed_dict={real_x: input_x_data})
        cv2.imwrite("output.jpg", check_imgs.reshape((width, height)) * 255)
