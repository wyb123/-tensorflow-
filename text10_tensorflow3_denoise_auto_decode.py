# 去噪自动编码器
# 在自动编码的基础上，训练数据加入噪声。输出的标签仍是原始的样本

# 去噪自编码网络的代码实现
# 在mnist集原始输入图片加入噪声。在自编码网络中进行训练
# 以得到抗干扰更强的特征提取模型
import numpy as np
...
def denoise_auto_encoder(_X, _weights, _biases, _keep_prob):
    ...
    layer_1out = tf.nn.dropout(layer_1, _keep_prob) #用两层dropout
    layer_2 = tf.nn.sigmoid(tf.add(tf.matmul(layer_1out, _weights['h2']), _biases['b2']))
    layer_2out = tf.nn.dropout(layer_2, _keep_prob) #用两层dropout
